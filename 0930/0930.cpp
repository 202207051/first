...
//상대 이동 속도 결정 보관 변수
int g_interval = 1000;
//음식의 상태 정보 보관 변수 : FALSE(일반), TRRE(레어)
BOOL g_rare;
//치트기 정보
BOOL g_cheat;
//나의 이동 속도를 보관할 변수
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		//랜덤 Seed값 변경
		srand(time(NULL));
		//나의 객체 좌표 설정 : 객치의 전체 범위가 결정 --> 140 * 140 크기의 정사각형
		g_me.left = 10;
		g_me.top = 10;
	...
	case WM_KEYDOWN:
	{
		...
		//화면 다시 그릭 OS에게 요청 : 그래야 변경된 좌표 그림 확인이 가능
		InvalidateRect(hWnd, NULL, TRUE);
	}
	break;
	case WM_COMMAND:
	...
	case WM_PAINT:
	{
		...
		//음식 객체 그리기 : 레어 음식 여부 같이
		if (g_rare) {


			HPEN myPen = CreatePen(PS_SOLID, 2, RGB(255, 255, 0));
		...
		//생명령 정보 출력
		WCHAR buf[128] = { 0, };
		wsprintf(buf, L"남은 생명력 : %d개", g_my_lift);
